...
namespace MMI_SP.Insurance.Policies
{
    internal static class InsurerData
    {
        ...
        internal static Option<VehicleData> FindVehicle(List<VehicleData> vehicles, string vehicleId)
        {
        ...
        {
            return FindVehicle(vehicles, vehicleId).match<Result<bool>>(
                onSome: vehicle =>
                {
        ...
        {
            string id = VehicleIdentifier.Get(veh);
            return FindVehicle(vehicles, id).match<Result<bool>>(
                onSome: existing => VehicleDataFactory.CreateFrom(veh, id)
                    .and_then(updated =>
                    {
        ...
        {
            return FindVehicle(vehicles, updatedData.Id).match<Result<bool>>(
                onSome: vehicle =>
                {
                    vehicles.Remove(vehicle);
